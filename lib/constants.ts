...
/** Temporary date ranges when booking is disabled (inclusive). Add more ranges as needed. */
...
export function getMinBookingDateStr(): string {
  ...
  while (
    isClosedBookingDate(
      `${base.getFullYear()}-${(base.getMonth() + 1).toString().padStart(2, "0")}-${base.getDate().toString().padStart(2, "0")}`
    )
  ) {
